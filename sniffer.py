"""From https://github.com/jpcsmith/wf-tools.
...
import logging
import threading
from typing import Optional, IO, List, Callable
import signal
import tempfile
import subprocess
...
class TCPDumpPacketSniffer(PacketSniffer):
    ...
    def __init__(
        ...
        snaplen: Optional[int] = None, command_name: Optional[str] = "tcpdump"
    ):
    ...
    def start(self) -> None:
        ...
        self._args = [
            self._command_name, '-n', '--buffer-size', str(self.buffer_size),
...
class MitmproxyPacketSniffer(PacketSniffer):
    """A wrapper around mitmproxy to perform traffic interception and request logging."""
    start_delay = 2
    # How long to wait before terminating the sniffer
    stop_delay = 2
    buffer_size = 4096
    ...
    def __init__(
        ...
        self._subprocess: Optional[subprocess.Popen] = None
        self._pcap: Optional[IO[bytes]] = None
        self._command_name: str = "mitmdump"
        self._iface: str = iface
        self._port: int  = 8080
        self._ip: str = ni.ifaddresses(iface)[ni.AF_INET][0]['addr']
        self._set_proxy: Callable = set_proxy
    ...
    def start(self) -> None:
        ...
        self._pcap = tempfile.NamedTemporaryFile(mode='rb', suffix='.mitmdump')
        self._args = [
            self._command_name, '-w', self._pcap.name, '-p', str(self._port),
        ]
            #self._command_name, '-n', '--buffer-size', str(self.buffer_size),
            #'--interface', self.interface, #'--dont-verify-checksums',
            #'--no-promiscuous-mode', '--snapshot-length', str(self.snaplen),
        ...
        self._subprocess = subprocess.Popen(
            self._args, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        time.sleep(self.start_delay)
        self._log.info("Started mitmproxy: '%s'", ' '.join(self._args))
        print(self._set_proxy(f"{self._ip}:{self._port}"))
        self._log.info(f"Configured proxy to {self._ip}:{self._port}")
    ...
    def _terminate(self) -> CompletedProcess:
        ...
        if self._subprocess.poll() is None:
            # Wait for tcpdump to flush, this may only work because it's in
            # packet-buffered & immediate modes
            self._log.info('Waiting %.2fs for mitmproxy to flush',
                           self.stop_delay)
            time.sleep(self.stop_delay)

            stdout, stderr = stop_process(
                self._subprocess, timeout=3, name="mitmproxy")
            return_code = 0
        else:
            self._log.debug("mitmproxy already terminated")
            stdout, stderr = self._subprocess.communicate()
            return_code = self._subprocess.poll()
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            self._log.fatal(
                "Mitmdump failed with error:\n%s", err.stderr.decode('utf-8'))
            raise
        else:
            self._log.info("Mitmdump complete.")
        finally:
            self._subprocess = None
            self._set_proxy(":0")
